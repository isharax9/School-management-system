	...
	if(isset($_POST['changepassword']))
	{
        ...
        if($password == "" || $newpassword == "" || $repassword ==""){
            ...
            function resetPasswordValidation(){
                $conn = getConnection();
                $sql = "select * from Officer where id='{$_SESSION['Officerid']}'";
            ...
            $user =  getUserById($_SESSION['Officerid']);
            $id = $user['id'];
            if(resetPasswordValidation() == $password){
                if(resetPasswordValidation() != $newpassword){
                    if($newpassword == $repassword){
                        if(strlen($newpassword) >= 8){
                                $check = updateOfficerPassword($id, $newpassword);
                                if($check){
                                    echo "Password changed!";
                                    header('location: ../View/viewOfficerProfile.php');
                                }else{
                                    echo "Error Occured!";
                                }
